function validate() {
    ...
    if (!exp.test(student_account) && student_account == "") {
    ...
    if (!exp.test(student_tel)&&student_tel == "") {
    ...
    if(!exp.test(student_password) && student_password == ""){
    ...
    var exp = new RegExp(/^(?!_)(?!.*?_$)([a-zA-Z]|[\u4e00-\u9fa5]){2,30}$/);
    if (!exp.test(student_college) && student_college == "") {
