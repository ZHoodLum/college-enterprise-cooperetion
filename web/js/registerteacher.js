function validate() {
    ...
    if (!exp.test(teacher_name)  && teacher_name == "") {
    ...
    if (!exp.test(teacher_tel) && teacher_tel == "") {
    ...
    if(!exp.test(teacher_password) && teacher_password == ""){
        alert("密码不符");
    ...
    if (!exp.test(teacher_job) && teacher_job == "") {
    ...
    var exp = new RegExp(/^(?!_)(?!.*?_$)([a-zA-Z]|[\u4e00-\u9fa5]){2,30}$/);
    if (!exp.test(teacher_college) && teacher_college == "") {
