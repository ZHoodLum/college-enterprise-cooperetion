...
//    查询数据总数量
    ...
    public List<StudentInfo> teacherFindAllStudentInfo(int teacherId) {
        List<StudentInfo> list = new ArrayList<StudentInfo>();
        String sql = "select * from studentinfo s,teacherinfo t,classinfo c,enterprise_studentinfo es,enterpriseinfo e where s.student_id = es.student_id  and es.enterprise_id = e.enterprise_id and c.class_id = s.class_id  and t.teacher_id = s.teacher_id and t.teacher_id = ?;";
        try{
            pstate = con.prepareStatement(sql);
...
//            if (!"".equals(studentInfo.getClassId())){
